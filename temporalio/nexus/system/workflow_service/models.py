...
class _SignalWithStartWorkflowRequestTransferTypeConverter(
    ...
    def from_transfer_type(
        ...
        if not value.signal_name:
            raise ValueError(
                "missing required field SignalWithStartWorkflowRequest.signal"
            )
        signal = value.signal_name
        return SignalWithStartWorkflowRequest(
            workflow=workflow,
            args=payloads_from_proto(value.input) if value.HasField("input") else None,
            id=id,
            task_queue=task_queue,
            signal=signal,
            signal_args=payloads_from_proto(value.signal_input)
            if value.HasField("signal_input")
            else None,
            execution_timeout=duration_from_proto(value.workflow_execution_timeout)
            if value.HasField("workflow_execution_timeout")
            else None,
            run_timeout=duration_from_proto(value.workflow_run_timeout)
            if value.HasField("workflow_run_timeout")
            else None,
            task_timeout=duration_from_proto(value.workflow_task_timeout)
            if value.HasField("workflow_task_timeout")
            else None,
            request_id=value.request_id if bool(value.request_id) else None,
            id_reuse_policy=workflow_id_reuse_policy_from_proto(
                value.workflow_id_reuse_policy
            ),
            id_conflict_policy=workflow_id_conflict_policy_from_proto(
                value.workflow_id_conflict_policy
            )
            if value.workflow_id_conflict_policy != 0
            else None,
            retry_policy=retry_policy_from_proto(value.retry_policy)
            if value.HasField("retry_policy")
            else None,
            cron_schedule=value.cron_schedule if bool(value.cron_schedule) else None,
            memo=memo_from_proto(value.memo) if value.HasField("memo") else None,
            search_attributes=search_attributes_from_proto(value.search_attributes)
            if value.HasField("search_attributes")
            else None,
            priority=priority_from_proto(value.priority)
            if value.HasField("priority")
            else None,
            versioning_override=versioning_override_from_proto(
                value.versioning_override
            )
            if value.HasField("versioning_override")
            else None,
            start_delay=duration_from_proto(value.workflow_start_delay)
            if value.HasField("workflow_start_delay")
            else None,
            user_metadata=_UserMetadataTransferTypeConverter().from_transfer_type(
                value.user_metadata, UserMetadata
            )
...
class _UserMetadataTransferTypeConverter(
    ...
    def from_transfer_type(
        ...
        return UserMetadata(
            static_summary=payload_from_proto(value.summary)
            if value.HasField("summary")
            else None,
            static_details=payload_from_proto(value.details)
            if value.HasField("details")
            else None,
        )
...
class _SignalWithStartWorkflowResponseTransferTypeConverter(
    ...
    def from_transfer_type(
        ...
        return SignalWithStartWorkflowResponse(
            ...
            started=value.started if bool(value.started) else None,
        )
